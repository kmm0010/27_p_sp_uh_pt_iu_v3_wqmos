...
directory <- '/Volumes/SSD Part2/_userDataSSD2/Experiments/kmDissKM/ibexFarm/25_p_sp_uh_pt_iu_v2_somqw/25_p_sp_uh_pt_iu_v2_somqw/results'
...
for (s in subjs) {
	...
	plot(s.result$RTadj, main=s, ylim=c(0, 5000), typ='l', col='darkblue')
	abline(0, 0)
}
...
14% of your response times were before the onset of the acoustic information for 'p' and 't'. And, 43% of your responses were just 200 milliseconds after this onset. The human brain generally needs about 200 milliseconds to just make a decision, and it takes an even longer time to physically click a mouse button (and the internet interface should be make this slower). So, it appears that you were just pressing buttons fast in order to get through much of the experiment rather than attending to the stimuli.
...
## for individual
s <- 'A3HAPMZV7CYDY8'  # (too fast, rejected)
...
problempeople <- c('A3HAPMZV7CYDY8')
...
par(mfrow=c(2, 3), mar= borders, cex= 0.4, family = tfamily)
for (s in subjCorrect) {
...
par(mfrow=c(2, 3), mar= borders, cex= 0.3, family = tfamily)
...
## sp=red, b=black
par(mfrow=c(2, 3), mar= borders, cex= 0.5, family = tfamily)
for (s in subjrankdf$s) {
	print(s)
	s.result <- responsesCorrect[ responsesCorrect $workerID == s, ]
	ccol <- 'darkblue'
